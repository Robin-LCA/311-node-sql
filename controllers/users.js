...
const getUserById = (req, res) => {
  ...
  let params = [id]  //you can have more than on ? in your query, and they have to be in order of use in the query
...
const createUserCallback = (req, res) => {
  ...
  //    when the id is created that we need for the user_id columns in the other two tables
  ...
  // asynchronous: run something until it finishes whenever, and goes on to run other stuff while that first thing may or may not be finished
  ...
  db.query(sql, params, (err,rows) => {
    if(err){
      ...
      // SECOND QUERY
      // I need the id of the user we just inserted. For this example, might have to use MAX(id) to get the id
        ...
        if(err){
          ...
          db.query(sql, params, (err, rows) => {
            if(err){
              ...
              // it worked
              res.json(rows); //this is final return
            }
...
// sync uses promises (async/await)
  ...
  // SECOND QUERY
  let getId = results.insertId; // from the previous query
